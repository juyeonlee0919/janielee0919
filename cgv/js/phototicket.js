...
(function ($) {
        ...
        load: function () {
            ...
            var flipPhotoTicketSlider = new Swiper('.myPhtoticketFlip .swiper-container', {
                ...
                on: {
                    init: function () {
                        var nWindowWidth = $(window).width();
                        var nPadding = (nWindowWidth - 230) /2;
            ...
            popLayerShowHide("icon_search.test", "popCompleted", "popFogBg");
            $('.popFogBg').on({
                click:function(e){
                    var $fogBg = $(e.target);
                    var $popLayer;
    ...
    switch (win.screenId) {
        ...
        case 'PT002': $(this).on(this['_' + win.screenId]); break; // 서브페이지(내 포토티켓)
        case 'PT003': $(this).on(this._PT003); break; // 서브페이지 (자랑하기)
        case 'PT004': $(this).on(this._PT004); break; // 서브페이지 (자랑하기)
        default: break;
    }
